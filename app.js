...
app.post('/login', async (req, res) => {
    ...
    const { username, password } = req.body;
      try {
        ...
        if (!user)
          return res.status(404).json({status: 404, message: "Väärä käyttäjänimi tai salasana"})
...
app.post('/signup', async (req, res) => {
    ...
    let newUser = new User({"username":username, "password":password, "taxPercentage": tax, "hourlyWage": wage})
...
app.post('/settings', async (req, res) => {
    ...
    await User.findOneAndUpdate({"username":username}, {hourlyWage: Number(wage), taxPercentage: Number(tax)})
    res.cookie('tax', tax)
  ...
  app.post('/', (req, res, next) => {
      ...
      python.on('close', (code) => {
          ...
          if (json_output.other.wasValidFile){
              Log.findByIdAndUpdate("6166db4319041439e9adccfd", 
              {$inc : {'converts' : 1}},
              {new: true},
              (err, response) => {
                  if (err){
                      ...
                      console.log("Convert logged, total converts: " + response.converts );
                  }  
